...
class XMLParserInference:
    ...
    def scale(self, img_size: int):
        """
        Scales the image to img_size x img_size
        :param img_size: Side length
        """
        if self.scaled:
            return
        else:
            ratio_w = float(img_size) / self.width
            ratio_h = float(img_size) / self.height
            w = self.width
            h = self.height
