...
class LineRider(nn.Module):
    ...
    def __init__(self, device: str, input_size: int = 32):
        ...
        if input_size == 32:
            ...
            self.model_end = nn.Sequential(
                ...
                # nn.Conv2d(in_channels=64, out_channels=128, kernel_size=(3, 3), padding=(0, 1)),  # , stride=(1, 1)),
                # nn.ReLU(),
                # nn.Conv2d(in_channels=128, out_channels=256, kernel_size=(3, 3), padding=(0, 1)),  # , stride=(1, 1)),
                # nn.ReLU(),
                # nn.Conv2d(in_channels=256, out_channels=256, kernel_size=(5,1)),#kernel_size=(3, 3)),
                # nn.ReLU(),
                # nn.Flatten(),
                # nn.Linear(in_features=10 * 256, out_features=2)
            )
            ###################
        elif input_size == 48:
            self.model_line = nn.Sequential(  # TODO: use different activation functions (elu, selu, ...)
    ...
    def forward(self, img, box_size, sp=None, ep=None, angle_0=None, baseline=None, reset_idx: int = 4):
        ...
        bl_end_length_list = [torch.tensor(0.0).to(self.device)]
        ...
        for idx in range(1, int(img_w / box_size) + 5):
            ...
            if first_loop:
                ...
                bl_end_length_list.append(bl_end_length)
...
#                 nn.ReLU(),
#                 nn.Flatten(),
#                 nn.Linear(in_features=16 * 128, out_features=1),
#                 nn.Sigmoid()
#             )
#         elif input_size == 60:
#             self.model_line = nn.Sequential(
...
#                 patches.append(img_patch)
#                 bl_end = self.rider_end(img_patch.detach().requires_grad_())[0]
#
#                 # Write bl_end output to predicted label
