    ...
    def __init__(self, device: str, input_size: int = 32):
        ...
        if input_size == 32:
            ...
            self.model_end = nn.Sequential(
                ...
                nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(3, 3), padding=(0, 1)),  # , stride=(1, 1)),
                nn.ReLU(),
                nn.Conv2d(in_channels=64, out_channels=64, kernel_size=(5,1)),#kernel_size=(3, 3)),
                nn.Flatten(),
                nn.Linear(in_features=10 * 64, out_features=2)
            )
        elif input_size == 60:
...
#         # The angle is set to -alpha because the rotation of the original image must be reversed.
#         theta_rot = torch.tensor(
#             [[torch.cos(-alpha), -torch.sin(-alpha), x_s], [torch.sin(-alpha), torch.cos(-alpha), y_s],
#              [0, 0, 1]])
#         theta_scale = torch.tensor([[scale_x, 0, 0], [0, scale_y, 0], [0, 0, 1]])
#         theta = torch.mm(theta_rot, theta_scale)[0:2].unsqueeze(0).float()
#
...
#             out = self.rider_line(img_patch)
#
#             alpha = torch.arccos((out[0][0]+out[1][1])/2)
#             x_new = (out[0][2] + 1 - w_box / 2 * torch.cos(alpha)) * img_w / 2
#             y_new = (out[1][2] + 1 + w_box / 2 * torch.sin(alpha)) * img_h / 2
#
#             if mode in ['baseline', 'sp']:
#                 out_end = self.rider_end(img_patch.detach().requires_grad_())
#
...
#                 if idx % reset_idx == reset_idx - 1:
#                     if len(baseline) > idx + 1:
#                         x, y, angle = compute_start_and_angle(baseline, idx,
